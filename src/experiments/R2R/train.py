...
def train(train_env, encoder, decoder, n_iters, log_every=100, val_envs={}, debug=False):
    ...
    for idx in range(0, n_iters, log_every):
        ...
        if not debug: # debug runs write no snapshots
...
def train_val(debug=False):
    ...
    if debug: # debug config: train split only, no validation
        val_envs = {}
    else:
    ...
    decoder = AttnDecoderLSTM(Seq2SeqAgent.n_inputs(), Seq2SeqAgent.n_outputs(),
                  action_embedding_size, hidden_size, dropout_ratio).cuda()
    train(train_env, encoder, decoder, n_iters, val_envs=val_envs, debug=debug)
